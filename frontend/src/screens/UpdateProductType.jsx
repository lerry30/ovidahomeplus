...
const UpdateProductType = () => {
    const currentName = zProductType(state => state.name);
    const currentImage = zProductType(state => state.image);

    const [data, setData] = useState({name: currentName});
    ...
    const productType = async () => {
        try {
            ...
            form.append('id', zProductType.getState()?.id);
            form.append('name', TitleFormat(name));
            form.append('file', image);

            const response = await sendForm(urls?.updateproducttype, form, 'PUT');
            if(response) {
    ...
    useLayoutEffect(() => {
        zProductType.getState()?.reloadProductTypeData();
    }, []);
